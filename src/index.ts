...
function coerceValue(raw: string, kind: 'number' | 'boolean' | 'null' | 'string' | undefined): CoercedValue {
  if (kind === 'string' || kind === undefined) return raw;
...
function autoCoerce(raw: string): CoercedValue {
  if (raw === '') return raw;
  if (raw === 'true') return true;
  if (raw === 'false') return false;
...
function applyCoercion(
  row: Row,
  headers: string[] | null,
  coerce: ParseOptions['coerce']
): CoercedValue[] {
  if (!coerce) return row;
  return row.map((raw, i) => {
    if (coerce === 'auto') return autoCoerce(raw);
 ...
 */
export class CsvStreamParser<T = Record<string, CoercedValue> | string[]> {
  private readonly delimiter: string;
  private readonly wantHeader: boolean;
  private readonly coerce: ParseOptions['coerce'];
  ...
  private endRow(produced: Row[]): void {
  ...
  private consumeRows(rows: Row[]): T[] {
    const result: T[] = [];
    for (const row of rows) {
      ...
      const finalValue: T = this.headers
        ? (this.buildRecord(coerced) as unknown as T)
        : (coerced as unknown as T);
  ...
  private buildRecord(coerced: CoercedValue[]): Record<string, CoercedValue> {
    const rec: Record<string, CoercedValue> = {};
 ...
 */
export function parse(input: string, options?: ParseOptions & { header: false }): CoercedValue[][];
export function parse(input: string, options?: ParseOptions & { header?: true }): Record<string, CoercedValue>[];
...
export default { parse, stringify, CsvStreamParser };
